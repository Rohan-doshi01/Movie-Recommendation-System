...
def recommend():
    ...
    for reviews in soup_result:
        if reviews.string:
            ...
            review_words_list = reviews.string.split(' ')

            p_words = open('./templates/positive-words.txt', 'r').read()
            p_words_list = p_words.split('\n')

            n_words = open('./templates/negative-words.txt', 'r').read()
            n_words_list = n_words.split('\n')

            # TODO: make new array and append both words_list to that array, then pass it to frontend

            if pred:
                for word in review_words_list:
                    if word in p_words_list:
                        words_to_highlight.append(word)

            else:
                for word in review_words_list:
                    if word in n_words_list:
                        words_to_highlight.append(word)
    ...
    return render_template('recommend.html',title=title,poster=poster,overview=overview,vote_average=vote_average,
        vote_count=vote_count,release_date=release_date,movie_rel_date=movie_rel_date,curr_date=curr_date,runtime=runtime,status=status,genres=genres,movie_cards=movie_cards,reviews=movie_reviews,casts=casts,cast_details=cast_details,words_to_highlight=words_to_highlight)
